...
func TestClient(t *testing.T) {
	...
	type testCase struct {
		testFunc func(t *testing.T, client *Client, iFunc *IndexFunction, indexName string)
	}
	tcs := map[string]testCase{
		"TestBulkUpdate": {func(t *testing.T, client *Client, iFunc *IndexFunction, indexName string) {
			// given
			...
			require.EventuallyWithT(t, func(c *assert.CollectT) {
				searchResponse := searchAllVulnerabilities(c, client, indexName)
		...
		"TestUpdate": {func(t *testing.T, client *Client, _ *IndexFunction, indexName string) {
			// given
			createDataInIndex(t, client, indexName, []*Vulnerability{&aVulnerability}, 1)
			updateRequest := `{
			  "query": {
				"match_all": {}
			  },
		...
		"TestAsyncDeleteByQuery": {func(t *testing.T, client *Client, _ *IndexFunction, indexName string) {
			// given
			createDataInIndex(t, client, indexName, []*Vulnerability{&aVulnerability}, 1)
		...
		"TestDeleteByQuery": {func(t *testing.T, client *Client, _ *IndexFunction, indexName string) {
			// given
			createDataInIndex(t, client, indexName, []*Vulnerability{&aVulnerability}, 1)
		...
		"TestSearch": {func(t *testing.T, client *Client, _ *IndexFunction, indexName string) {
			// given
			createDataInIndex(t, client, indexName, []*Vulnerability{&aVulnerability}, 1)
		...
		"TestSearchStream": {func(t *testing.T, client *Client, _ *IndexFunction, indexName string) {
	...
	for testName, testCase := range tcs {
		t.Run(testName, func(t *testing.T) {
			...
			indexName := "test_" + strings.ToLower(rand.Text())
			t.Cleanup(func() {
				err := iFunc.DeleteIndex(indexName)
				require.NoError(t, err, "failed to delete index")
			})

			schema := folder.GetContent(t, "testdata/testSchema.json")
			err = iFunc.CreateIndex(indexName, []byte(schema))
			require.NoError(t, err)
			testCase.testFunc(t, client, iFunc, indexName)
		})
	}
}
...
func searchAllVulnerabilities(c *assert.CollectT, client *Client, indexName string) *SearchResponse[*Vulnerability] {
